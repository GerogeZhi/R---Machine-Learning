...
#Non-lienar Transformations of the Predictors
...
LoadLibraries=function(){
  library(ISLR)
  library(MASS)
  print('The libraries has been loaded')
}
LoadLibraries
LoadLibraries()
...
for (i in 1:10){
  ...
  cv.prob[i]=mean(knn.pred!=Direction.2005)
}
cv.prob
plot(cv.prob)
...
lines(age.grid,pred1$fit,col='red',lwd=2)
title('Smoothing Spline')
fit=smooth.spline(age,wage,df=16)
fit2=smooth.spline(age,wage,cv=TRUE)
fit2$df
lines(fit,lwd=2,col='red',lty='dashed')
lines(fit2,lwd=2,col='blue')
legend('topright',legend=c('4 DF','--16 DF','6.8 DF'),col=c('red','red','blue'),lty=1,lwd=2,cex=.8)
...
km.out$cluster
plot(x,col=(km.out$cluster+1),main='k-means clustering ressults with k=2',xlab='',ylab='',pch=20,cex=2)
...
plot(hclust(dd,method='complete'),main='compelte linakge with correlation-based distance',xlab='',sub='')
...
pve
par(mfrow=c(1,2))
plot(pve,type='o',ylal='PVE',xlab='Principle component',col='blue')
...
plot(hclust(data.dist),label=nci.labs,main='complete linkage',xlab='',sub='')
...
hc.out=hclut(dist(pr.out$x[,1:5]))
plot(hc.out,labes=nci.labs,main='hier.clust. on first five score vectors')
